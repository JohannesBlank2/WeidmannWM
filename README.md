# Weidmann WM Poker Edition - Spieleshow-App

Lokale Spieleshow für 5 Einzelspieler. Ein Windows-Laptop ist Server,
TV/Beamer zeigt `/display`, jeder Spieler nutzt `/play` auf dem Handy, und die
Show wird über `/admin` gesteuert. Alles läuft im lokalen WLAN ohne
Build-Schritt.
...
Beim Start zeigt das Terminal lokale IP, fertige URLs und QR-Codes für `/play`
und `/admin`.
...
| `/play/` | Handy-Ansicht: Spieler beitreten, Kategorie wählen, Spin auslösen, Spielcontent |
...
server/
  ...
  registry.js  Lädt alle Spiele aus /games und normalisiert Metadaten
  sockets.js   Socket.IO-Events für Admin, Handy, Buzzer und Spiele
...
Der Server ist die Single Source of Truth. Jede Änderung am State wird per
Socket.IO als `state` an alle Clients gesendet und regelmäßig in `state.json`
geschrieben.

## Rundenablauf

Es gibt 5 Runden für 5 Einzelspieler. In Runde 1 ist Spieler 1 dran, in Runde 2
Spieler 2, usw. Jeder Spieler wählt einmal eine Kategorie.
...
1. Runde und auswählender Spieler werden auf dem TV angezeigt.
2. Admin öffnet die Kategorie-Auswahl.
3. Der auswählende Spieler wählt auf dem Handy eine Kategorie.
4. Die Registry legt bis zu drei ungespielte Spiele dieser Kategorie in den Spin.
5. Der Spieler drückt auf dem Handy `SPIN`.
6. Die TV-Ansicht zeigt den Glücksrad-/Slot-Spin und lost serverseitig ein Spiel aus.
7. Nach der Auslosung setzen die Spieler auf dem Handy geheim 0 bis 50 Coins auf
   einen Mitspieler.
8. Admin startet das ausgeloste Spiel.
9. Beim Beenden wird das Spiel als gespielt markiert und später nicht mehr vorgeschlagen.
10. Admin trägt die Platzierungen 1 bis 5 ein und wendet die Auszahlung an.
...
- `player.gameScore`: temporäre Platzpunkte für die aktive Auswertung.
...
Solange `built: false` gesetzt ist, lädt die UI das Demo-Spiel `buzzer-test` als
Platzhalter. Nur Spiele mit `selectable: true` können vom Spin wirklich
ausgelost werden; alle anderen bleiben als Decoys im Rad sichtbar. Der Kern muss
für neue Spiele nicht angepasst werden.
